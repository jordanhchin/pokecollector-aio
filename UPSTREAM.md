...
* Durable upstream paths are backups, auth/JWT data, Pokédex images, scan uploads, scan traces, and debug logs. AIO configures each beneath `/config` and stores PostgreSQL there too. The interactive API's hardcoded `/app/backups` is linked to `/config/backups`; `BACKUP_DIR` puts pre-upgrade dumps there; and `JWT_SECRET_FILE` explicitly places the signing key in `/config/auth`.
* AIO retains upstream nginx's headers, CSP, gzip behavior, limits, cache rules, and timeouts. Only its listen port and backend target change for the single-container topology.
