...
> **Architecture support:** Current releases support AMD64/x86-64 hosts, including x86-64 Unraid systems. ARM64 is not currently published or supported; it will require an architecture-safe frontend build and dedicated smoke testing before being added.
...
Back up first, pull a chosen tag, remove the old container, and recreate it with the same `/config` mount and environment. Releases publish `latest`, the repository release tag (for example `1.2.0`), major/minor (`1.2`), and major (`1`) for `linux/amd64`. Prefer a full version tag for reproducibility. `latest` tracks the newest packaging release, not the upstream default branch. Never reuse or overwrite an existing release tag; publish a new version when packaging changes.
